...
//      let currentSegmentColor = 'rgb(0, 123, 255)';  // Standardfarbe
      ...
      coords.forEach((pt, idx) => {
        ...
        if (pointsData[idx] && pointsData[idx].type === 'bus' && 
          ...
          inBusSection = true;
          currentSegmentColor = 'rgb(255, 165, 0)'; // Farbe für Busabschnitte (Orange)
        } else if (inBusSection) {
          // Wenn wir aus einem Busabschnitt herauskommen, setzen wir das Flag zurück
          inBusSection = false;
          currentSegmentColor = 'rgb(0, 123, 255)'; // Zurück zur Standardfarbe
        }
        ...
        if (!inBusSection) {
          ...
          chartData.push({ x: totalDistance, y: ele, color: currentSegmentColor });
        }
      });
      ...
      const datasets = [
        {
          ...
          borderColor: (context) => {
            // Dynamische Farbgebung der Linie auf Basis des Busabschnitts
            const index = context.dataIndex;
            return chartData[index] ? chartData[index].color : 'rgb(0, 123, 255)';
          },
          fill: false,
          tension: 0.1,
          pointRadius: 0,
